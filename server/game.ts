...
export function createGame(
  ...
  namespace.on("connection", (socket: Socket) => {
    ...
    socket.on("answer", (answerData) => {
      const question = gameState.questions?.find(
        (q) => q.prompt === answerData.prompt
      );
      if (question) {
        const playerAnswer = question.answers.find(
          (a) => a.player.id === answerData.clientId
        );
        if (playerAnswer) {
    ...
    socket.on("vote", (voteData) => {
      ...
      const votingPlayer = gameState.players.find(
      ...
      const votedAnswer = question.answers.find(
        (answer) => answer.player.id === votedForID
      );
      if (!votedAnswer) {
  ...
  function computeScoreForQuestion(questionNumber: number) {
